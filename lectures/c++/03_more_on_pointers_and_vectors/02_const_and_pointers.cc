...
int main() {
  ...
  const int* pc{&a}; //helps the compiler to optimize things, because if it know that you will never change it it performs better -> constant pointer: you cannot change it after deferentiating it -> when I deferentiate it, it becomes a const int so I cannot change it
  // *pc = 7; 	// error
