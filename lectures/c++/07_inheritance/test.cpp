...
int main(){
    ...
    //Output: (in case of a void class foo)
    //1
    //1
    ...
    // 8
    // 8
    // virtual table added by the compiler -> every time a new children is inserted, the virtual table has a row for sizeof the children
    // v+
    // foo  -> foo::bar()
    // derived -> derived::bar()

    // don't make virtual a function that is called many many times! -> you have to jump in the memory!!! And that's bad
}