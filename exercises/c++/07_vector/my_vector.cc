...
class Vector {
	...
	void check_and_increase_capacity(){
		if (_capacity == 0)
			reserve(8); // reserve(1); -> memory allocation is expensive! At the beginning in this case I have to make a lot of re-allocations!
...
public:
	auto size() const noexcept { return _size; }  // I'm returning by value so there is no possibility that the user modify the value of _size
	auto capacity() const noexcept { return _capacity; }  // const is a statement that I will not modify the value of the variable inside the function
	...
	// T& operator[](const std::size_t i) const noexcept { -> here the compiler check that in the function you don't modify the value of the element, but the function returns a reference so you can modfy the value of the element: NONSENSE!
	...
	void push_back(T&& x){ // since c++11
		...
		elem[_size] = std::move(x); // call the move semantic of your type if it exists, otherwise passes to copy
		++_size;
	}
	...
	// for printing std::vector since we overloaded it
	template <typename T>
...
struct Date{
	...
	Date(int d, int m, int y): d{d}, m{m}, y{y} {} // there is no confusion between the two d's, m's and y's!
};
